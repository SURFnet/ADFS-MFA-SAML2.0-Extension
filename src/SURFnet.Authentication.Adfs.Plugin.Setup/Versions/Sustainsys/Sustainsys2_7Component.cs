...
namespace SURFnet.Authentication.Adfs.Plugin.Setup.Versions.Sustainsys
{
    ...
    /// </summary>
    public class Sustainsys2_7Component : SustainsysBaseComponent
    {
        public Sustainsys2_7Component()
            : base("Sustainsys.Saml2 v2.7 from Metadata")
        {
            this.ConfigParameters = new string[]
            {
                ConfigSettings.IdPEntityId, ConfigSettings.IdPMdFilename, ConfigSettings.SPEntityId,
        ...
        {
            var rc = base.Install(); // first regular install
        ...
        protected override int ExctractSustainsysConfig(List<Setting> settings)
        {
            ...
            var sustainsysCfgPath = FileService.OurDirCombine(FileDirectory.AdfsDir, SetupConstants.SustainCfgFilename);
            ...
            var sustainsysSection = sustainsysConfig.Descendants(XName.Get(SustainsysSaml2Section))
                                                    .FirstOrDefault();

            // SP entityID
            foundvalue = sustainsysSection?.Attribute(XName.Get(EntityId))
                                          ?.Value;
            ...
            var spcerts = sustainsysSection?.Descendants(SPCerts)
                                           .FirstOrDefault();
            if (spcerts == null)
            {
                LogService.Log.Error("spcerts==null");
            }

            var firstcert = spcerts?.Descendants(XName.Get("add"))
                                   .FirstOrDefault();
            if (firstcert == null)
            {
                LogService.Log.Error("firstcert == null");
            }

            foundvalue = firstcert?.Attribute(XName.Get(CertFindValue))
                                  ?.Value;
            if (foundvalue == null)
            {
                LogService.Log.Error("foundvalue == null");
            }

            settings.SetFoundSetting(ConfigSettings.SPPrimarySigningThumbprint, foundvalue);

            // get the first IdP from the list
            var identityProviders = sustainsysSection?.Descendants(IdentityProviders)
                                                     .FirstOrDefault();
            var identityProvider = identityProviders?.Descendants(XName.Get("add"))
                                                    .FirstOrDefault();

            // IdP entityID attribute
            foundvalue = identityProvider?.Attribute(XName.Get(EntityId))
                                         ?.Value;
            ...
            if (x == null)
            {
                LogService.Log.Error("x == null");
            }

            foundvalue = x?.Value;
            if (foundvalue.StartsWith("~/"))
            {
                foundvalue = foundvalue.Substring(2);
            }
