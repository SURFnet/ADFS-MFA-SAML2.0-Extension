...
function Install-SigningCertificate{
    ...
    if($CertificateFile){
        $certificatePassword = Read-host "Geef het wachtwoord op voor $CertificateFile" -AsSecureString
        $selfSignedCertificate = Import-PfxCertificate -FilePath "$InstallDir\$CertificateFile" -CertStoreLocation Cert:\LocalMachine\My -Exportable -Password $certificatePassword
    }
    else{
