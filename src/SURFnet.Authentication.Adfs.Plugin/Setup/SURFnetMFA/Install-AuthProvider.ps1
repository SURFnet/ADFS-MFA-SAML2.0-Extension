...
function Install-AuthProvider {
	Param(
		...
		[string]
		$AssemblyName,
		[Parameter(Mandatory = $true, HelpMessage = "Type name of the AD FS MFA plugin")]
		[string]
		$TypeName
	)
	...
	try {
		...
		# TODO: We will no longer install files in the GAC
		...
		$deviceRegistrationService = Get-WmiObject win32_service | Where-Object { $_.name -eq "drs" }
